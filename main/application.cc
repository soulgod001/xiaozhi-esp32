...
void Application::OnWakeWordDetected(const std::string& wake_word_invoke) {
    if (!protocol_) {
        return;
    }
    ...
    if (device_state_ == kDeviceStateIdle) {
        ...
        std::string wake_word = wake_word_invoke.empty() ? audio_service_.GetLastWakeWord() : wake_word_invoke;
...
void Application::OnWakeWordDetected() {
    OnWakeWordDetected(""); // 调用新版本，传入空字符串表示使用默认唤醒词
}
